# リモート GPU サーバーで train_alpha.py を実行し、.onnx/.pt をコピーバックする
param(
...
Write-Host "=== Running Alpha training on ${REMOTE_HOST} ===" -ForegroundColor Cyan
Write-Host "  args: $argsStr" -ForegroundColor Gray
& ssh @SSH_OPTS -t $REMOTE_HOST "cd ~/${REMOTE_DIR} && ${REMOTE_PYTHON} tools/train_alpha.py $argsStr"
