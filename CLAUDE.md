...
- **evaluation** — `Evaluator` extracts 74 features (piece counts, hand pieces, king safety, attack maps, PST, king shelter, pawn structure, mobility, threats, zone control, tactical summaries) and computes score as `dot(features, weights)` (linear) or MLP forward pass (74→128→64→1 with ReLU). MLP weights loaded from text file via `loadMlp()`. Features 28-31,37 are /100 normalized. `buildAttackMap()` uses Bitboard-based `countAttackers()`. Optional heavy features (index 36-41) require full legal move generation per side.
