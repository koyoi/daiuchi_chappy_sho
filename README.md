...
現状は、局面特徴量に対する線形評価関数、浅い alpha-beta 探索、対局結果からのオンライン学習を持っています。勝った側の指し手を模倣し、負けた側の指し手を抑制する方向に `random-shogi.weights` を更新します。
...
現在の探索には、時間制御、短い詰み探索、静止探索、手の並べ替え、置換表、評価内の利き計算キャッシュ、ルート並列探索が入っています。

## 実装範囲

合法手生成では、駒の移動、成り、不成、持ち駒打ち、自玉を王手にさらす手の除外、二歩、行き所のない駒打ち、打ち歩詰めの除外を扱っています。

千日手、入玉宣言、時間管理、CSAサーバへのTCP接続、詰み専用探索、静止探索、ニューラルネット評価は未実装です。
